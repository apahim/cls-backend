-- Consolidated CLS Backend Database Schema
-- This migration contains the complete final schema for the CLS backend system
-- Consolidated from migrations 001-004: complete schema + reactive reconciliation + fan-out architecture
-- Includes: clusters, nodepools, status tracking, cluster-centric reconciliation, organization multi-tenancy

-- =============================================================================
-- CORE TABLES
-- =============================================================================

-- Clusters table - Core cluster lifecycle management
CREATE TABLE IF NOT EXISTS clusters (
    ...
    CONSTRAINT clusters_name_organization_unique UNIQUE(name, organization_domain),
    CONSTRAINT clusters_name_organization_id_unique UNIQUE(name, organization_id)
);

-- NodePools table - Cluster nodepool management
...
-- Organizations table for multi-tenant organization management
...
-- Cluster ownership for organization-based access control
CREATE TABLE IF NOT EXISTS cluster_ownership (
    ...
    UNIQUE(cluster_id, organization_domain)
);

-- Organization mapping for domain/ID conversion
CREATE TABLE IF NOT EXISTS organization_mapping (
    id SERIAL PRIMARY KEY,
    organization_domain VARCHAR(255) NOT NULL UNIQUE,
...
-- Core table indexes
CREATE INDEX IF NOT EXISTS idx_clusters_organization_domain ON clusters(organization_domain);
...
CREATE INDEX IF NOT EXISTS idx_cluster_ownership_organization_domain ON cluster_ownership(organization_domain);
CREATE INDEX IF NOT EXISTS idx_cluster_ownership_organization_id ON cluster_ownership(organization_id);
CREATE INDEX IF NOT EXISTS idx_organization_mapping_id ON organization_mapping(organization_id);
...
CREATE OR REPLACE FUNCTION get_organization_id_from_domain(domain VARCHAR(255))
RETURNS VARCHAR(100) AS $$
DECLARE
    org_id VARCHAR(100);
BEGIN
    SELECT organization_id INTO org_id
    FROM organization_mapping
    WHERE organization_domain = domain;

    RETURN COALESCE(org_id, domain);
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Map organization ID to domain (for backward compatibility)
CREATE OR REPLACE FUNCTION get_organization_domain_from_id(org_id VARCHAR(100))
RETURNS VARCHAR(255) AS $$
DECLARE
    domain VARCHAR(255);
BEGIN
    SELECT organization_domain INTO domain
    FROM organization_mapping
    WHERE organization_id = org_id;

    RETURN COALESCE(domain, org_id);
END;
$$ LANGUAGE plpgsql IMMUTABLE;
...
RETURNS TRIGGER AS $$
BEGIN
    -- If organization_id is not set but organization_domain is, populate it
    IF NEW.organization_id IS NULL AND NEW.organization_domain IS NOT NULL THEN
        NEW.organization_id := get_organization_id_from_domain(NEW.organization_domain);
    END IF;

    -- If organization_domain is not set but organization_id is, populate it
    IF NEW.organization_domain IS NULL AND NEW.organization_id IS NOT NULL THEN
        NEW.organization_domain := get_organization_domain_from_id(NEW.organization_id);
    END IF;
...
-- List clusters by organization ID
CREATE OR REPLACE FUNCTION list_clusters_by_organization_id(
...
BEGIN
    ...
    WHERE c.organization_id = p_organization_id
      AND c.deleted_at IS NULL
      AND (p_platform IS NULL OR c.spec->>'platform' = p_platform)
      AND (p_status IS NULL OR c.status->>'phase' = p_status)
    ORDER BY c.created_at DESC
    LIMIT p_limit OFFSET p_offset;
END;
$$ LANGUAGE plpgsql;

-- Count clusters by organization ID
CREATE OR REPLACE FUNCTION count_clusters_by_organization_id(
    p_organization_id VARCHAR(100),
    p_platform VARCHAR(50) DEFAULT NULL,
    p_status VARCHAR(50) DEFAULT NULL
) RETURNS INT AS $$
DECLARE
    cluster_count INT;
BEGIN
    SELECT COUNT(*)::INT INTO cluster_count
    FROM clusters c
    WHERE c.organization_id = p_organization_id
      AND c.deleted_at IS NULL
      AND (p_platform IS NULL OR c.spec->>'platform' = p_platform)
...
-- Get cluster by ID and organization ID
CREATE OR REPLACE FUNCTION get_cluster_by_id_and_organization(
    p_cluster_id UUID,
    p_organization_id VARCHAR(100)
) RETURNS TABLE (
    id UUID,
...
-- Organization information view
CREATE OR REPLACE VIEW organization_info AS
SELECT
    om.organization_id,
    om.organization_domain,
    om.display_name,
    COUNT(c.id) as cluster_count,
    om.created_at,
    om.updated_at
FROM organization_mapping om
LEFT JOIN clusters c ON c.organization_id = om.organization_id AND c.deleted_at IS NULL
GROUP BY om.organization_id, om.organization_domain, om.display_name, om.created_at, om.updated_at;
...
CREATE TRIGGER update_organization_mapping_updated_at
    BEFORE UPDATE ON organization_mapping
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Organization field synchronization trigger
CREATE TRIGGER sync_organization_fields_trigger
...
-- Insert default organization mappings
INSERT INTO organization_mapping (organization_domain, organization_id, display_name) VALUES
('redhat.com', 'redhat-com', 'Red Hat Organization'),
('acme.com', 'acme-corp', 'ACME Corporation'),
('startupxyz.com', 'startup-xyz', 'Startup XYZ')
...
COMMENT ON TABLE clusters IS 'Core cluster lifecycle management with organization multi-tenancy';
